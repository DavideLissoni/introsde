...
public class HealthProfileReader {
	...
	static
    {
		...
		Person pallo = new Person("Pinco","Pallo", myDate);
		HealthProfile hp = new HealthProfile(68.0,1.72);
		Person john = new Person("John","Doe",myDate);
	...
	public void createPerson(String firstname, String lastname, Date birthday){
		Person newperson = new Person(firstname,lastname, birthday);
		database.put(newperson.getFirstname()+" "+newperson.getLastname(), newperson);	
	}
	public HealthProfile displayHealthProfile(String id){
		return database.get(id).gethProfile();}
	
	public void updateHealthProfile(String id,Double height, Double weight){
		database.get(id).gethProfile().setHeight(height);
		database.get(id).gethProfile().setWeight(weight);}
